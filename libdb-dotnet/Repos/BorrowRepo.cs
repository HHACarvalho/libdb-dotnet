...
namespace libdb_dotnet.Repos
{
    public class BorrowRepo : CoreRepo<Borrow>, IBorrowRepo
    {
        ...
        {
            return await _dbs.Skip((pageNumber - 1) * pageSize).Take(pageSize).Include(x => x.BookEntry).ThenInclude(x => x.Book).Include(x => x.Member).ToListAsync();
        }

        public async Task<Borrow?> FindOne(int id)
        {
            return await _dbs.Where(x => x.Id.Equals(id)).Include(x => x.BookEntry).ThenInclude(x => x.Book).Include(x => x.Member).FirstOrDefaultAsync();
        }
    }
}